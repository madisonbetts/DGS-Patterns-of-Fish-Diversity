# Preliminary filtering and plotting for objective 1
# cgh

# install/load pacakges
packages <- c("sf", "rnaturalearth", 
              "colorspace", "taxize", "phytools",
              "ggplot2", "ape", "dplyr", 
              "readxl", "fishtree")
...
# some plots quick
He_lat <- ggplot(dat, aes(x = Latitude, y = He)) +
  geom_point() +
...
He_gdr <- ggplot() +
  ...
  geom_point(data = dat, aes(x = Longitude, y = Latitude, color = He))+
