...
# - Save CACO_1_fst and CACO_1_coords to data/CACO-1.RData
# ============================================================
...
base_dir <- "/Users/johnmccall/Library/CloudStorage/OneDrive-TheOhioStateUniversity/Spring_2026/Landgen_DGS/DGS-Patterns-of-Fish-Diversity/DGS-Patterns-of-Fish-Diversity/Objective 2/Data/CACO-1"
...
vcf_file  <- file.path(base_dir, "Catostomus.vcf")
...
CACO_1_fst <- as.matrix(fst_out)
storage.mode(CACO_1_fst) <- "numeric"

CACO_1_fst[is.na(CACO_1_fst)] <- 0
CACO_1_fst <- (CACO_1_fst + t(CACO_1_fst)) / 2
CACO_1_fst[CACO_1_fst < 0] <- 0
diag(CACO_1_fst) <- 0

pop_order <- c("lower", "upper")
pop_order <- pop_order[pop_order %in% rownames(CACO_1_fst)]
CACO_1_fst <- CACO_1_fst[pop_order, pop_order, drop = FALSE]

rownames(CACO_1_fst) <- colnames(CACO_1_fst) <- as.character(seq_len(nrow(CACO_1_fst)))
...
p_map <- ggplot() +
  ...
  theme_classic() +
  labs(
    title = "CACO-1 sampling populations",
    x = "Longitude",
    y = "Latitude"
  )
...
# 9) quick IBD plot
# -----------------------------
coord_mat <- as.matrix(CACO_1_coords[, c("lon", "lat")])
geo_dist_km <- geosphere::distm(coord_mat, fun = geosphere::distHaversine) / 1000

upper_idx <- upper.tri(CACO_1_fst)

ibd_df <- data.frame(
  fst = CACO_1_fst[upper_idx],
...
save(
  CACO_1_fst,
  CACO_1_coords,
  file = file.path(data_dir, "CACO-1.RData")
)

assign("gl", gl, envir = .GlobalEnv)
assign("CACO_1_fst", CACO_1_fst, envir = .GlobalEnv)
assign("CACO_1_coords", CACO_1_coords, envir = .GlobalEnv)
...
cat("FST matrix dimensions: ", nrow(CACO_1_fst), " x ", ncol(CACO_1_fst), "\n", sep = "")
cat("RData saved to: ", file.path(data_dir, "CACO-1.RData"), "\n", sep = "")
cat("=================================================\n\n")
