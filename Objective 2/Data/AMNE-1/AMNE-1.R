...
# - Save AMNE_1_fst and AMNE_1_coords to data/AMNE-1.RData
# ============================================================
...
base_dir <- "/Users/johnmccall/Library/CloudStorage/OneDrive-TheOhioStateUniversity/Spring_2026/Landgen_DGS/DGS-Patterns-of-Fish-Diversity/DGS-Patterns-of-Fish-Diversity/Objective 2/Data/AMNE-1"
...
vcf_file  <- file.path(base_dir, "Ameiurus.vcf")
...
AMNE_1_fst <- as.matrix(fst_out)
storage.mode(AMNE_1_fst) <- "numeric"

AMNE_1_fst[is.na(AMNE_1_fst)] <- 0
AMNE_1_fst <- (AMNE_1_fst + t(AMNE_1_fst)) / 2
AMNE_1_fst[AMNE_1_fst < 0] <- 0
diag(AMNE_1_fst) <- 0

pop_order <- c("lower", "upper")
pop_order <- pop_order[pop_order %in% rownames(AMNE_1_fst)]
AMNE_1_fst <- AMNE_1_fst[pop_order, pop_order, drop = FALSE]

rownames(AMNE_1_fst) <- colnames(AMNE_1_fst) <- as.character(seq_len(nrow(AMNE_1_fst)))
...
p_map <- ggplot() +
  ...
  theme_classic() +
  labs(
    title = "AMNE-1 sampling populations",
    x = "Longitude",
    y = "Latitude"
  )
...
# 9) quick IBD plot
# -----------------------------
coord_mat <- as.matrix(AMNE_1_coords[, c("lon", "lat")])
geo_dist_km <- geosphere::distm(coord_mat, fun = geosphere::distHaversine) / 1000

upper_idx <- upper.tri(AMNE_1_fst)

ibd_df <- data.frame(
  fst = AMNE_1_fst[upper_idx],
...
save(
  AMNE_1_fst,
  AMNE_1_coords,
  file = file.path(data_dir, "AMNE-1.RData")
)

assign("gl", gl, envir = .GlobalEnv)
assign("AMNE_1_fst", AMNE_1_fst, envir = .GlobalEnv)
assign("AMNE_1_coords", AMNE_1_coords, envir = .GlobalEnv)
...
cat("FST matrix dimensions: ", nrow(AMNE_1_fst), " x ", ncol(AMNE_1_fst), "\n", sep = "")
cat("RData saved to: ", file.path(data_dir, "AMNE-1.RData"), "\n", sep = "")
cat("=================================================\n\n")
