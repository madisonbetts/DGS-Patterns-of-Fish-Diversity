...
##
## Focal lineage: Little River Bass lineage
##
## This script:
...
# paths
# -----------------------------
out_dir  <- file.path("/Users/johnmccall/Library/CloudStorage/OneDrive-TheOhioStateUniversity/Spring_2026/Landgen_DGS/DGS-Patterns-of-Fish-Diversity/DGS-Patterns-of-Fish-Diversity/Objective 2/Data", "LRBA-1")
data_dir <- file.path(out_dir, "data")

geno_xlsx <- file.path(out_dir, "genotype_data.xlsx")
meta_xlsx <- file.path(out_dir, "metadata.xlsx")
...
dataset_code   <- "LRBA-1"
focal_taxon    <- "LRB"
river_order    <- c("Blackfork_Creek", "Honobia_Creek", "Little_River", "Glover_River", "Big_Eagle_Creek", "Upper_Mountain_Fork", "Mountain_Fork", "Pero_Creek_Rolling_Fork_trib", "Cossatot_River_AR", "Western_Saline_River_AR")
site_name_map  <- c("Blackfork_Creek" = "Blackfork Creek", "Honobia_Creek" = "Honobia Creek", "Little_River" = "Little River", "Glover_River" = "Glover River", "Big_Eagle_Creek" = "Big Eagle Creek", "Upper_Mountain_Fork" = "Upper Mountain Fork", "Mountain_Fork" = "Mountain Fork", "Pero_Creek_Rolling_Fork_trib" = "Pero Creek", "Cossatot_River_AR" = "Cossatot River", "Western_Saline_River_AR" = "Western Saline River")
drop_stage1_n  <- c("Blackfork_Creek" = 3, "Little_River" = 1, "Glover_River" = 5, "Upper_Mountain_Fork" = 2, "Mountain_Fork" = 2, "Western_Saline_River_AR" = 9)
drop_stage2_n  <- c("Upper_Mountain_Fork" = 8, "Mountain_Fork" = 7)
drop_stage3_n  <- c("Western_Saline_River_AR" = 1)
...
fit_internal_centroids <- function(meta_df, geno_df, loci, species_levels = c("NB", "LRB", "OB")) {
  ...
  # standardize genotype strings and make missing explicit so rows are not dropped
  geno_chr[] <- lapply(geno_chr, function(x) {
    x <- as.character(x)
    x[x %in% c(NA, "NA", "<NA>", "unknown", "", "missing")] <- NA_character_
    x[is.na(x)] <- "MISSING"
    x
  })

  # retain only loci with >= 2 observed non-missing genotype states
  keep_cols <- vapply(geno_chr, function(x) {
    obs <- unique(x[x != "MISSING"])
    length(obs) >= 2
  }, logical(1))

  geno_chr <- geno_chr[, keep_cols, drop = FALSE]

  if (ncol(geno_chr) == 0) {
    stop("No informative within-CIH loci remained for stage 3 centroid fitting.")
  }

  cat("Stage 3 informative loci retained:", ncol(geno_chr), "\n")

  mm_list <- lapply(geno_chr, function(x) {
    x <- factor(x)
  ...
  if (nrow(mm) != nrow(meta_df)) {
    stop("Stage 3 model matrix row count does not match metadata row count.")
  }
...
if (length(drop_stage3_n) > 0) {
  cat("Stage 3 candidate loci:", length(cih_loci), "\n")
  cat("NB rows:", sum(meta_s2$putative_taxon == "NB"), "\n")
  cat("LRB rows:", sum(meta_s2$putative_taxon == "LRB"), "\n")
  cat("OB rows:", sum(meta_s2$putative_taxon == "OB"), "\n")
...
p_map <- ggplot() +
  ...
  coord_quickmap(
    xlim = range(site_lookup$lon, na.rm = TRUE) + c(-8, 8),
    ylim = range(site_lookup$lat, na.rm = TRUE) + c(-5, 5)
  ) +
  theme_classic() +
  labs(
    x = "Longitude",
...
save(
  LRBA_1_fst,
  LRBA_1_coords,
  file = file.path(data_dir, paste0(dataset_code, ".RData"))
)
