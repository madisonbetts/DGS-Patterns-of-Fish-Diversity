...
#
# Saves to:
#   /Users/johnmccall/Library/CloudStorage/OneDrive-TheOhioStateUniversity/Spring_2026/Landgen_DGS/DGS-Patterns-of-Fish-Diversity/DGS-Patterns-of-Fish-Diversity/Objective 2/Data/LEME-3/data/LEME-3.RData
#
# Plots in RStudio:
#   - site map
...
LEME_3_fst <- matrix(
  c(
    0.0, 0.8127, 0.9401, 0.9596, 0.9678, 0.9714,
    0.8127, 0.0, 0.6131, 0.6460, 0.6771, 0.6703,
    0.9401, 0.6131, 0.0, 0.2545, 0.4937, 0.3061,
    0.9596, 0.6460, 0.2545, 0.0, 0.5155, 0.0732,
    0.9678, 0.6771, 0.4937, 0.5155, 0.0, 0.7159,
    0.9714, 0.6703, 0.3061, 0.0732, 0.7159, 0.0
  ),
  nrow = 6,
  byrow = TRUE,
  dimnames = list(LEME_3_coords$site, LEME_3_coords$site)
)

LEME_3_fst[LEME_3_fst < 0] <- 0
diag(LEME_3_fst) <- 0
...
diag(LEME_3_rivdists) <- 0

# -----------------------------
# 4) checks
# -----------------------------
stopifnot(identical(rownames(LEME_3_fst), LEME_3_coords$site))
stopifnot(identical(colnames(LEME_3_fst), LEME_3_coords$site))
stopifnot(identical(rownames(LEME_3_rivdists), LEME_3_coords$site))
stopifnot(identical(colnames(LEME_3_rivdists), LEME_3_coords$site))
stopifnot(isTRUE(all.equal(LEME_3_fst, t(LEME_3_fst))))
stopifnot(isTRUE(all.equal(LEME_3_rivdists, t(LEME_3_rivdists))))

# -----------------------------
# 5) pairwise dataframe for IBD
# -----------------------------
ibd_df <- data.frame(
  site1 = rownames(LEME_3_fst)[row(LEME_3_fst)[upper.tri(LEME_3_fst)]],
  site2 = colnames(LEME_3_fst)[col(LEME_3_fst)[upper.tri(LEME_3_fst)]],
  fst = LEME_3_fst[upper.tri(LEME_3_fst)],
  rivdist_km = LEME_3_rivdists[upper.tri(LEME_3_rivdists)],
  stringsAsFactors = FALSE
)
...
plot_sites <- left_join(LEME_3_coords, site_lookup, by = "site")
...
# 8) save objects
# -----------------------------
LEME_3_rivdists = LEME_3_rivdists / 1000
