...
# paths
# -----------------------------
base_dir <- "/Users/johnmccall/Library/CloudStorage/OneDrive-TheOhioStateUniversity/Spring_2026/Landgen_DGS/DGS-Patterns-of-Fish-Diversity/DGS-Patterns-of-Fish-Diversity/Objective 2/Data/ONMY-4"
data_dir <- file.path(base_dir, "data")

geno_file <- file.path(base_dir, "calledgenos_minInd50_95p.csv")
out_rdata <- file.path(data_dir, "ONMY-4.RData")

# -----------------------------
# 1) read called genotype matrix
# -----------------------------
geno_raw <- read_csv(geno_file, show_col_types = FALSE)
...
geno_hf <- cbind(
  pop = factor(keep_pops, levels = keep_groups),
  geno_hf
)

# -----------------------------
# 4) pairwise FST
# hierfstat may return row/col names as either group labels
# or underlying numeric group codes. assign names safely,
# then reorder if possible.
# -----------------------------
ONMY_4_fst <- pairwise.WCfst(geno_hf)
ONMY_4_fst <- as.matrix(ONMY_4_fst)

if (is.null(rownames(ONMY_4_fst)) ||
    all(rownames(ONMY_4_fst) %in% as.character(seq_len(nrow(ONMY_4_fst))))) {
  rownames(ONMY_4_fst) <- keep_groups
  colnames(ONMY_4_fst) <- keep_groups
}

if (all(keep_groups %in% rownames(ONMY_4_fst))) {
  ONMY_4_fst <- ONMY_4_fst[keep_groups, keep_groups, drop = FALSE]
}
...
# replace FST names with numeric site IDs to match coords
rownames(ONMY_4_fst) <- colnames(ONMY_4_fst) <- as.character(ONMY_4_coords$site_id)
