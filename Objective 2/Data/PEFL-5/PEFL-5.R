...
# - Save PEFL_5_fst and PEFL_5_coords to data/PEFL-5.RData
# ============================================================
...
base_dir <- "/Users/johnmccall/Library/CloudStorage/OneDrive-TheOhioStateUniversity/Spring_2026/Landgen_DGS/DGS-Patterns-of-Fish-Diversity/DGS-Patterns-of-Fish-Diversity/Objective 2/Data/PEFL-5"
...
vcf_file  <- file.path(base_dir, "Perca.vcf")
...
PEFL_5_fst <- as.matrix(fst_out)
storage.mode(PEFL_5_fst) <- "numeric"

PEFL_5_fst[is.na(PEFL_5_fst)] <- 0
PEFL_5_fst <- (PEFL_5_fst + t(PEFL_5_fst)) / 2
PEFL_5_fst[PEFL_5_fst < 0] <- 0
diag(PEFL_5_fst) <- 0

pop_order <- c("lower", "upper")
pop_order <- pop_order[pop_order %in% rownames(PEFL_5_fst)]
PEFL_5_fst <- PEFL_5_fst[pop_order, pop_order, drop = FALSE]

rownames(PEFL_5_fst) <- colnames(PEFL_5_fst) <- as.character(seq_len(nrow(PEFL_5_fst)))
...
p_map <- ggplot() +
  ...
  theme_classic() +
  labs(
    title = "PEFL-5 sampling populations",
    x = "Longitude",
    y = "Latitude"
  )
...
# 9) quick IBD plot
# -----------------------------
coord_mat <- as.matrix(PEFL_5_coords[, c("lon", "lat")])
geo_dist_km <- geosphere::distm(coord_mat, fun = geosphere::distHaversine) / 1000

upper_idx <- upper.tri(PEFL_5_fst)

ibd_df <- data.frame(
  fst = PEFL_5_fst[upper_idx],
...
save(
  PEFL_5_fst,
  PEFL_5_coords,
  file = file.path(data_dir, "PEFL-5.RData")
)

assign("gl", gl, envir = .GlobalEnv)
assign("PEFL_5_fst", PEFL_5_fst, envir = .GlobalEnv)
assign("PEFL_5_coords", PEFL_5_coords, envir = .GlobalEnv)
...
cat("FST matrix dimensions: ", nrow(PEFL_5_fst), " x ", ncol(PEFL_5_fst), "\n", sep = "")
cat("RData saved to: ", file.path(data_dir, "PEFL-5.RData"), "\n", sep = "")
cat("=================================================\n\n")
