...
# legend placed inside the southeast U.S./Atlantic side of the map
# axes numbers and ticks removed
# north arrow labeled with "N"
# saves succinct PDF + PNG outputs
# -------------------------------
library(readxl)
library(dplyr)
library(ggplot2)
...
out_dir <- "/Users/johnmccall/Library/CloudStorage/OneDrive-TheOhioStateUniversity/Spring_2026/Landgen_DGS/DGS-Patterns-of-Fish-Diversity/DGS-Patterns-of-Fish-Diversity/Objective 2/analyses/QC_plot_of_map"
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

png_file <- file.path(out_dir, "sites_family_map.png")
pdf_file <- file.path(out_dir, "sites_family_map.pdf")
...
p <- ggplot() +
  ...
  guides(
    color = guide_legend(
      ...
      title.position = "top",
      title.hjust = 0,
      override.aes = list(size = 0.9, alpha = 1)
    )
  ) +
  theme_classic() +
  theme(
    ...
    legend.key.width = unit(0.045, "in"),
    legend.spacing.x = unit(0.01, "in"),
    legend.spacing.y = unit(0.003, "in"),
