# Brook trout (SAFO) FINAL
# Reads SAFO datasets from data_dir
...
out_dir <- "/Users/johnmccall/Library/CloudStorage/OneDrive-TheOhioStateUniversity/Spring_2026/Landgen_DGS/DGS-Patterns-of-Fish-Diversity/DGS-Patterns-of-Fish-Diversity/Objective 2/analyses/pilot_ibd/SAFO/"

study_dirs <- paste0("SAFO-", 1:10)
...
# helper functions
# -----------------------------
read_safo_study <- function(study_code, data_dir) {
  ...
  if (!"site_id" %in% names(coords)) {
    coords$site_id <- seq_len(nrow(coords))
  }
...
all_pairs <- map_dfr(study_dirs, function(x) {
  message("Reading ", x)
  study_to_pairs(read_safo_study(x, data_dir))
}) %>%
...
all_pairs <- all_pairs %>%
  mutate(
    study_code = factor(study_code, levels = study_order)
  ) %>%
  arrange(study_code)
...
# -----------------------------
ggsave(
  filename = file.path(out_dir, "SAFO_all_studies_fst_vs_distance.png"),
  plot = p,
  width = 9,
  height = 6,
  dpi = 300
...
write.csv(
  all_pairs,
  file = file.path(out_dir, "SAFO_all_pairs.csv"),
  row.names = FALSE
)
